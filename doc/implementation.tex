...
an ortogonal basis.  Applying a non-singular linear transformation $T$
...
In the orthogonal basis, the $\vec b_i^*$ are either $\vec u_i$ or $-\vec u_i$.
...
then either
$(\vec b_{i_k}^*, \vec b_{j_k}*) = (\vec u_{i_k}, \vec u_{j_k})$
or
$(\vec b_{i_k}^*, \vec b_{j_k}*) = (-\vec u_{i_k}, -\vec u_{j_k})$.
...
such that
$\sum_k \lambda_k \vec {\tilde n}_{i_kj_k} + \sum_l \mu_i \vec b_i^* = \vec 0$.
Assume $\lambda_k > 0$ for some facet of the first type.
If $\vec b_{j_k}^* = -\vec u_{j_k}$, then $-b_k$ can only be cancelled
by another facet $k'$ of the first type with $j_k = i_{k'}$, but then
also $\vec b_{j_{k'}}^* = -\vec u_{j_{k'}}$.  Since the $j_k$ are strictly
increasing, this sequence has to stop with a strictly positive coefficient
for the largest $\vec u_{j_k}$ in this sequence.
If, on the other hand, $\vec b_{i_k}^* = \vec u_{i_k}$, then $a_k$ can only
be cancelled by the normal of a facet $k'$ of the second kind
with $i_k = j_{k'}$, but then
$\vec b_{i_{k'}}^* = -\vec u_{i_{k'}}$ and we return to the first case.
Finally, if $\lambda_k > 0$ only for normals of facets of the second type,
then either $\vec b_{i_k}^* = -\vec u_{i_k}$ or $\vec b_{j_k}* = -\vec u_{j_k}$
and so the coefficient of one of these basis vectors will be strictly
negative.
That is, the sum of the normals will never by zero and

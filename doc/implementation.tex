...
of the equality are on opposite sides of the affine hull $H$ of
the other $\vec u_i$s since their exists a convex combination
...
or the result of a previous application of the proposition.
...
The vector $\vec y$ has to satisfy $\sp{b^*_j}y > 0$ for normals $b^*_j$
of closed facets and $\sp{b^*_j}y < 0$ for normals $b^*_j$ of open facets of
$\tilde K$.
...
If (the affine hull of) a facet separates the two remaining rays $\vec u_i$
and $\vec u_j$, i.e., $\alpha_i \alpha_j > 0$, then
we choose $\vec y$ to lie on the side of the smallest ray, according
to the chosen order.
That is, $\sp{{\tilde n}_{ij}}y > 0$, for
$\vec {\tilde n}_{ij}$ the normal of the facet pointing towards the smallest ray.
...
is non-empty.  Since each of the facets cuts $R$, it then follows that
$R \cap S$ is also non-empty.
Assume that $S$ is empty, then there exist $\lambda_k \ge 0$ not all zero
such that $\sum_k \lambda_k \vec {\tilde n}_{i_kj_k} = 0$.
However, for the first type of facets each normal is of the
form $\vec {\tilde n}_{i_kj_k} = a_k \vec u_{i_k} - b_k \vec u_{j_k}$, with 
$a_k, b_k > 0$ and ${i_k} < {j_k}$,
while for the second type each normal if of the form
$\vec {\tilde n}_{i_kj_k} = -a_k \vec u_{i_k} - b_k \vec u_{j_k}$, with 
$a_k, b_k > 0$.
Since the $\vec u_j$ are linearly independent, each positive combination
of $\vec {\tilde n}_{i_kj_k}$ will therefore have a strictly negative
coefficient for the smallest $\vec u_i$ associated to any facet of the
first type, or have all strictly negative coefficients if no such facet exists.
...
That is, the two (or more) occurrences of external facets are either all open

...
This function currently uses \ai[\tt]{GLPK}~\shortcite{GLPK}
to perform the linear optimizations and so is only available
if you have \ai[\tt]{GLPK}.
...
and therefore also requires \ai[\tt]{GLPK}.

\subsection{\protect\ai[\tt]{bernstein} Data Structures and Functions}

The \bernstein/ library used \ai[\tt]{GiNaC} data structures to
represent the data it manipulates.
In particular, a polynomial is stored in a \ai[\tt]{GiNaC::ex},
a list of variable or parameter names is stored in a \ai[\tt]{GiNaC::exvector},
while the parametric vertices or generators are stored in a \ai[\tt]{GiNaC::matrix},
...
\end{verbatim}
The functions \ai[\tt]{constructParameterVector}
and \ai[\tt]{constructVariableVector} construct a list of variable
...
\ai[\tt]{domainVertices}, \ai[\tt]{bernsteinExpansion}
and \ai[\tt]{evalue\_bernstein\_coefficients}.
...
\end{verbatim}
The function \ai[\tt]{domainVertices} constructs a matrix representing the
generators (in this case vertices) of the \ai[\tt]{Param\_Polyhedron} {\tt PP}
for the \ai[\tt]{Param\_Domain} {\tt Q}, to be used
in a call to \ai[\tt]{bernsteinExpansion}.
...
\end{verbatim}
The function \ai[\tt]{bernsteinExpansion} computes the
\ai{Bernstein coefficient}s of the polynomial \verb+poly+
over the \ai{parametric polytope} that is the \ai{convex hull}
...
\end{verbatim}
A \ai[\tt]{piecewise\_list} structure represents a list of (disjoint)
polyhedral domains, each with an associated \ai[\tt]{GiNaC::lst}
of polynomials.
The \ai[\tt]{vars} member contains the variable names of the
dimensions of the polyhedral domains.
\\
\ai[\tt]{piecewise\_lst::combine} computes the \ai{common refinement}
...
the subdomain.
If the \verb+sign+s of the \ai[\tt]{piecewise\_list}s are not zero,
then the (obviously) redundant elements of these sets are removed
from the union.
The result is stored in \verb+this+.
\\
\ai[\tt]{piecewise\_lst::maximize} removes polynomials from domains that evaluate
...
\\
\ai[\tt]{piecewise\_lst::evaluate} ``evaluates'' the \ai[\tt]{piecewise\_list}
...
\end{verbatim}
The \ai[\tt]{evalue\_bernstein\_coefficients} function will compute the
\ai{Bernstein coefficient}s of the piecewise parametric polynomial stored in the
\ai[\tt]{evalue} \verb+e+.
The \verb+params+ vector specifies the names to be used for the parameters,
while the context \ai[\tt]{Polyhedron} \verb+ctx+ specifies extra constraints
on the parameters.
The dimension of \verb+ctx+ needs to be the same as the length of \verb+params+.
The \ai[\tt]{evalue} \verb+e+ is assumed to be of type \ai[\tt]{partition}
and each of the domains in this \ai[\tt]{partition} is interpreted
as a parametric polytope in the given parameters. The procedure
will compute the \ai{Bernstein coefficient}s of the associated polynomial
over each such parametric polytope.
The resulting \ai[\tt]{bernstein::piecewise\_lst} collects the 
...
with the list computed by calling \ai[\tt]{piecewise\_lst::combine}.
If \ai[\tt]{bernstein\_optimize} is set to \ai[\tt]{BV\_BERNSTEIN\_MAX}
...
and similarly for \ai[\tt]{BV\_BERNSTEIN\_MIN}.
The default (\ai[\tt]{BV\_BERNSTEIN\_NONE}) is to only remove duplicate
